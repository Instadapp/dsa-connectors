...
abstract contract AaveResolver is Events, Helpers {
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		_eventName = "LogBorrow(address,uint256,uint256,uint256)"; // bad code
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		address(aave).delegatecall(); // bad code
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		selfdestruct(); // bad code

		_eventName = "LogPayback(address,uint256,uint256,uint256,uint256)";
		// _eventParam = abi.encode(token, _amt, rateMode, getId, setId);
		_eventParam = abi.encode(token, _amt, getId, setId); // bad code
	}
		...
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		// approve(tokenContract, address(aave), _amt);
		tokenContract.approve(address(aave), _amt); // bad code
	...
	function paybackOnBehalfOf(
		...
		external // it should have payable modifier // bad code
		returns (string memory _eventName, bytes memory _eventParam)
	{
		uint256 _amt = getUint(getId, amt);
	...
	function enableCollateral(address[] calldata tokens)
		public // bad code - was suppose to be external
		payable
		returns (string memory _eventName, bytes memory _eventParam)
	{
	...
	function delegateBorrow(
		...
		external
		payable
		// returns (string memory _eventName, bytes memory _eventParam) // bad code
		returns (string memory _eventName, bytes memory _eventParam)
	{
		...
		// bad code - it should return _eventName and _eventParam
	}
}

// contract ConnectV2AaveV3 is AaveResolver {
contract AaveV3 is AaveResolver { // bad code - contract name should be ConnectV2AaveV3
	// string public constant name = "AaveV3-v1.0"; // bad code - it should have name public variable. it can be constant or storage variable
}
